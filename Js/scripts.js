...
function fetchProfile (url) {
    fetch(url)
        ...
        .then(data => {displayCard(data); getModals(data);addEvents(); closeModal()})
        .then()
...
function addEvents () {
    for (i=0; i<cards.length; i++) {
        cards[i].addEventListener('click', (e) => {
            const clickedCardIndex = [...cards].indexOf(e.currentTarget);
            const clickedModal = modalArray[clickedCardIndex];
            gallery.insertAdjacentHTML('afterbegin', clickedModal)
